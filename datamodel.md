...
serveren. Svaret indeholder også `afdeling_ubehandlede` — antallet af
**ubehandlede** sager (status `MODTAGET`, dvs. modtaget men endnu ikke taget
under behandling) for hele afdelingen, uafhængigt af `omfang` og af
hastegrad-/kategori-filtrene — så brugerfladen kan vise afdelingens ubearbejdede
indbakke som en tæller uden selv at regne.
